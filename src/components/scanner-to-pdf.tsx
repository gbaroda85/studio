...
export default function ScannerToPdf() {
  ...
  // 6 Dots Logic: 4 Corners + 2 Mid-Width points for better alignment
  const [points, setPoints] = useState<Point[]>([
      { x: 10, y: 10 }, { x: 90, y: 10 },
      { x: 90, y: 50 }, { x: 90, y: 90 }, // Extra mid-right point
      { x: 10, y: 90 }, { x: 10, y: 50 }  // Extra mid-left point
  ]);
  ...
  const onImageLoad = (e: SyntheticEvent<HTMLImageElement>) => {
    ...
    // Auto-edge heuristic: If image is portrait, set points slightly tighter for common documents
    if (height > width) {
        setPoints([
            { x: 12, y: 8 }, { x: 88, y: 8 },
            { x: 88, y: 50 }, { x: 88, y: 92 },
  ...
  // 3. PERSPECTIVE LOGIC (Always uses corner 4 points for Homography)
  const solvePerspective = (src: Point[], dst: Point[]) => {
    const p = [];
    // We only use points 0, 1, 3, 4 (the four corners) for the homography matrix
  ...
  const applyCorrection = async (): Promise<string> => {
    ...
    // Apply Magic Enhancement by default
  ...
  const handleConfirmAdjustment = async () => {
    ...
    toast({ title: "Page Added", description: "Corrected and stored." });
  };
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (pos) {
        setPoints(prev => {
            ...
            // If dragging corners, mid-points should follow? No, let's keep them independent for extreme precision
            return next;
        });
    }
  ...
  const handleBuildPdf = async () => {
    ...
    toast({ title: "PDF Ready", description: "High-quality scan bundle downloaded." });
  };
                    ...
                    <Card className="border-2 shadow-2xl overflow-hidden bg-black relative rounded-2xl md:rounded-[3rem]">
                        <CardHeader className="bg-muted/30 border-b py-2 px-6 flex flex-row items-center justify-between no-print">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Studio Viewfinder</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="p-0 relative aspect-[4/3] flex items-center justify-center overflow-hidden">
                                ...
                                <div className="p-12 text-center space-y-6 text-white w-full">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3 justify-center">
                                            <Button onClick={startCamera} className="bg-primary rounded-xl font-black uppercase text-[10px]">Retry</Button>
                            ...
                            {hasCameraPermission === true && (
                                <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                                    <div className="size-[80%] border-2 border-dashed border-white/40 rounded-xl">
                            ...
                            <div className="absolute bottom-6 right-6 md:right-10 md:bottom-10 z-20">
                                <Button variant="secondary" className="h-10 md:h-12 rounded-xl font-black uppercase text-[9px] shadow-2xl ring-4 ring-black/20" onClick={() => fileInputRef.current?.click()}>
                                    <ImageIcon className="mr-2 size-3.5" /> UPLOAD IMAGE
                                </Button>
                            </div>
                        </CardContent>
                        <CardFooter className="p-4 md:p-10 bg-white dark:bg-slate-950 border-t flex flex-col sm:flex-row gap-4">
                            <Button onClick={handleCapture} className="h-16 flex-[2] bg-gradient-button text-white font-black text-xl rounded-2xl shadow-2xl group transition-all">
                                <Zap className="mr-3 size-6 text-yellow-400 group-hover:scale-125 transition-transform" /> CAPTURE SCAN
                            </Button>
                    ...
                    <Card className="border-2 shadow-2xl border-primary/10 flex flex-col bg-card/50 rounded-[2rem] min-h-[400px]">
                        <CardHeader className="bg-primary/5 border-b p-6 flex flex-row items-center justify-between">
                            <CardTitle className="text-lg font-black uppercase tracking-tighter flex items-center gap-2">
                                <FileDigit className="size-5 text-primary" /> Scan Stack
                            </CardTitle>
                        ...
                        <CardContent className="flex-1 p-6">
                            {scannedPages.length === 0 ? (
                                <div className="py-20 text-center opacity-30 space-y-4">
                                    ...
                                    <p className="text-[10px] font-black uppercase">Stack is Empty</p>
                                </div>
                            ) : (
                        ...
                        <CardFooter className="p-6 bg-muted/10 border-t">
                            <Button disabled={scannedPages.length === 0} className="w-full h-14 bg-primary font-black text-sm rounded-xl shadow-xl active:scale-95" onClick={handleBuildPdf}>
                                {isBuildingPdf ? <Loader2 className="animate-spin mr-2"/> : <Download className="mr-2 size-5" />} SAVE BUNDLE PDF
                            </Button>
                        </CardFooter>
                    </Card>
                </div>
            ...
            <Card className="w-full max-w-5xl mx-auto shadow-2xl border-none overflow-hidden rounded-[2.5rem] animate-in zoom-in-95 duration-500">
                ...
                <CardContent className="p-0 bg-slate-200 dark:bg-slate-900 flex flex-col items-center justify-center min-h-[450px] md:min-h-[650px] relative overflow-hidden select-none"
                    ...
                    <div className="mt-8 flex items-center gap-3 px-6 py-3 bg-black/70 backdrop-blur-xl rounded-full text-white text-[10px] font-black uppercase tracking-widest border border-white/10 shadow-2xl z-40">
                        ...
                        {cropMode === 'rect' ? "Drag box to any area" : "Drag corners & side handles to edges"}
                    </div>
                </CardContent>
                <CardFooter className="bg-white dark:bg-slate-950 border-t p-6 md:p-8 flex justify-between gap-4">
                    <Button variant="ghost" onClick={() => setStage('viewfinder')} className="font-black text-[10px] uppercase h-12 px-8 rounded-xl">CANCEL</Button>
                    <div className="flex gap-3 flex-1 justify-end">
                         <Button onClick={() => setPoints([{x:10, y:10}, {x:90, y:10}, {x:90, y:50}, {x:90, y:90}, {x:10, y:90}, {x:10, y:50}])} variant="outline" className="h-12 px-6 border-2 font-black text-[10px] uppercase rounded-xl">Reset Points</Button>
            ...
            <div className="flex flex-col items-center gap-10 animate-in fade-in duration-500">
                ...
                <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                     ...
                     <button onClick={() => setStage('viewfinder')} className="aspect-[3/4] rounded-[2.5rem] border-4 border-dashed border-primary/20 flex flex-col items-center justify-center gap-4 hover:bg-primary/5 hover:border-primary transition-all group">
                        ...
                        <span className="text-sm font-black uppercase tracking-widest text-primary/40">Add More Pages</span>
                     </button>
                </div>
                ...
                <Card className="w-full max-w-2xl bg-slate-900 text-white rounded-[2.5rem] p-10 shadow-2xl border-t border-white/10">
                     <div className="flex flex-col md:flex-row items-center justify-between gap-10">
                        <div className="space-y-2 text-center md:text-left">
                            ...
                            <p className="text-slate-400 text-sm font-medium">Ready to export as high-quality PDF.</p>
                        </div>
                        <Button 
