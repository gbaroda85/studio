...
export default function ScannerToPdf() {
  ...
  const handleConfirmAdd = async (goToNext = false) => {
    ...
    const newPage: ScannedPage = { id, originalSrc: currentRawImage!, processedSrc: highRes, points, isScanned: true, originalIndex, vAlign: sourceData?.vAlign || 'center' };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
        ...
        for (let i = 0; i < scannedPages.length; i++) {
            ...
            let y;
            if (p.vAlign === 'top') y = 0;
            else if (p.vAlign === 'bottom') y = pageHeight - fh;
            else y = (pageHeight - fh) / 2;

            pdf.addImage(p.processedSrc, 'JPEG', x, y, fw, fh, undefined, 'FAST');
        }
            ...
            <div className="grid lg:grid-cols-12 gap-8 items-stretch animate-in slide-in-from-bottom-6 duration-500 w-full px-4 max-w-[1800px] mx-auto h-auto">
                ...
                <Card className="lg:col-span-5 border-2 shadow-xl overflow-hidden rounded-[3rem] bg-card flex flex-col h-full">
                    ...
                    <CardFooter className="p-4 md:p-6 border-t bg-[#f0f9f9] dark:bg-slate-800 flex-col gap-6 shrink-0">
                        ...
                        <div className="w-full space-y-4">
                            <Label className="text-[10px] font-black uppercase opacity-60">Vertical Alignment</Label>
                            <div className="grid grid-cols-3 gap-2">
                                <Button 
                                    variant={editingId && (pendingPages.find(p => p.id === editingId)?.vAlign === 'top' || scannedPages.find(p => p.id === editingId)?.vAlign === 'top') ? 'default' : 'outline'} 
                                    className="h-10 text-[9px] font-black uppercase"
                                    onClick={() => {
                                        const setV = (prev: ScannedPage[]) => prev.map(p => p.id === editingId ? { ...p, vAlign: 'top' as VAlign } : p);
                                        setPendingPages(setV); setScannedPages(setV);
                                    }}
                                >
                                    <AlignVerticalJustifyStart className="size-3 mr-1.5" /> TOP
                                </Button>
                                <Button 
                                    variant={editingId && (pendingPages.find(p => p.id === editingId)?.vAlign === 'center' || scannedPages.find(p => p.id === editingId)?.vAlign === 'center') ? 'default' : 'outline'} 
                                    className="h-10 text-[9px] font-black uppercase"
                                    onClick={() => {
                                        const setV = (prev: ScannedPage[]) => prev.map(p => p.id === editingId ? { ...p, vAlign: 'center' as VAlign } : p);
                                        setPendingPages(setV); setScannedPages(setV);
                                    }}
                                >
                                    <AlignVerticalJustifyCenter className="size-3 mr-1.5" /> CENTER
                                </Button>
                                <Button 
                                    variant={editingId && (pendingPages.find(p => p.id === editingId)?.vAlign === 'bottom' || scannedPages.find(p => p.id === editingId)?.vAlign === 'bottom') ? 'default' : 'outline'} 
                                    className="h-10 text-[9px] font-black uppercase"
                                    onClick={() => {
                                        const setV = (prev: ScannedPage[]) => prev.map(p => p.id === editingId ? { ...p, vAlign: 'bottom' as VAlign } : p);
                                        setPendingPages(setV); setScannedPages(setV);
                                    }}
                                >
                                    <AlignVerticalJustifyEnd className="size-3 mr-1.5" /> BOTTOM
                                </Button>
                            </div>
                        </div>
                    </CardFooter>
