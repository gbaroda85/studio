...
import { ShieldCheck, HelpCircle, Layout, MonitorCheck, Layers, UploadCloud, Download, RotateCw } from 'lucide-react';
            ...
            <section className="space-y-10 py-10 border-t">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    <div className="p-8 bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 shadow-xl space-y-4 hover:border-primary/50 transition-all">
                        ...
                        <h3 className="font-black uppercase text-sm tracking-widest text-slate-800 dark:text-white">Live Preview</h3>
                        <p className="text-xs text-muted-foreground leading-relaxed font-semibold">See your document in a browser-native preview box before you download. No more guesswork or repeated downloads.</p>
                    </div>
                    <div className="p-8 bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 shadow-xl space-y-4 hover:border-blue-500/50 transition-all">
                        <Layers className="text-blue-500 size-10" />
                        <h3 className="font-black uppercase text-sm tracking-widest text-slate-800 dark:text-white">High-DPI Export</h3>
                        <p className="text-xs text-muted-foreground leading-relaxed font-semibold">Our engine renders pages in high-definition (300 DPI equivalent) to ensure scanned text and signatures remain perfectly readable.</p>
                    </div>
                    <div className="p-8 bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 shadow-xl space-y-4 hover:border-teal-500/50 transition-all">
                        <ShieldCheck className="text-teal-500 size-10" />
                        <h3 className="font-black uppercase text-sm tracking-widest text-slate-800 dark:text-white">100% Client-Side</h3>
                        <p className="text-xs text-muted-foreground leading-relaxed font-semibold">We respect your data. Every byte of the PDF is generated in your computer's RAM. No files ever leave your device.</p>
                    </div>
                </div>
            </section>
        </div>
