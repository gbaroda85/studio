...
export async function removeBackground(input: RemoveBackgroundInput): Promise<RemoveBackgroundOutput> {
  const {media} = await ai.generate({
    ...
    prompt: [
        ...
        {text: 'Remove the background from this image. The main subject should be perfectly preserved. The output must be a PNG image with a transparent background.'},
    ],
    config: {
